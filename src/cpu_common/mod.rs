        ...
        self.cpu_infos
            ...
            .map(|cpu| {
                let target = match self.util_max {
                    ...
                    Some(util_max) => {
                        let util_tracking_sugg_freq = (cur_freq_max as f64 * util_max / 0.5) as isize; // min_util: 50%

                        #[cfg(debug_assertions)]
                        debug!(
                            "util: {util_max}, cur_freq_max: {cur_freq_max}, util_tracking_sugg_freq: {util_tracking_sugg_freq}"
                        );
